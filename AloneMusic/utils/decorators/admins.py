        ...
        if not is_non_admin and message.from_user.id not in SUDOERS:
            ...
            if message.from_user.id not in admins:
                if await is_skipmode(message.chat.id):
                    ...
                    upl = InlineKeyboardMarkup(
                        [[InlineKeyboardButton(text="ᴠᴏᴛᴇ", callback_data=f"ADMIN UpVote|{chat_id}_{MODE}")]]
                    )
...
» {upvote} ᴠᴏᴛᴇs ɴᴇᴇᴅᴇᴅ ғᴏʀ ᴛʜɪs ᴀᴄᴛɪᴏɴ."""
...
def AdminActual(mystic):
    async def wrapper(client, message):
        ...
        if message.from_user.id not in SUDOERS:
            ...
            if not member or not getattr(member.privileges, "can_manage_video_chats", False):
                return await message.reply_text(_["general_4"])
